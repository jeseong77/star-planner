...
const AddProblemBottomSheet: React.FC<AddProblemBottomSheetProps> = ({
  ...
  const handleAddProblem = useCallback(async () => {
    ...
    try {
      // Call the store action with just the name
      const newProblemObject = await addProblemAction(trimmedName);
      if (newProblemObject) {
        // Check if problem was successfully created
        Keyboard.dismiss();
        handleInternalClose(); // This will call onClose, which triggers parent's state change
      } else {
  ...
  }, [problemName, addProblemAction, handleInternalClose]);
    ...
    <>
      ...
      <Animated.View
        ...
        onLayout={(event) => {
          ...
          if (
            Math.abs(sheetInternalHeight.value - height) > 1 ||
            sheetInternalHeight.value === 0
          ) {
